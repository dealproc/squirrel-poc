                ...
                try {
                    using (var updateMgr = new UpdateManager(settings.UpdateUrl, settings.PackageId, FrameworkVersion.Net45)) {
                        ...
                        var updateInfo = await updateMgr.CheckForUpdate(ignoreDeltaUpdates: false,
                            progress: (progess) => {
                                Dispatcher.Invoke(() => txtSystemMessages.AppendText(string.Format("Download progress: {0}", progess) + Environment.NewLine));
                            }
                        );
                ...
                using (var updateMgr = new UpdateManager(settings.UpdateUrl, settings.PackageId, FrameworkVersion.Net45)) {
                    try {
                        ...
                        var releasesApplied = await updateMgr.UpdateApp((progess) => {
                            Dispatcher.Invoke(() => txtSystemMessages.AppendText(string.Format("Download progress: {0}" + Environment.NewLine, progess)));
                        });
